...
class DinitzAlgorithmVisualizer(Scene):
    ...
    def _update_text_generic(self, text_attr_name, new_text_str, font_size, weight, color, play_anim=True):
        ...
        new_mobj.set_z_index(old_mobj.z_index) # Ensure new mobject has same z_index as old
        ...
        if play_anim:
            ...
            if not is_new_mobj_in_group:
                ...
                if new_text_str != "" and new_mobj not in self.mobjects:
                    ...
                    self.remove(new_mobj) # Remove new standalone mobject if it's empty but was on scene
    ...
    def _dfs_recursive_find_path_anim(self, u, pushed, current_path_info_list):
        ...
        self.play(Create(highlight_ring), run_time=0.3) # Slightly longer ring creation
        self.wait(0.5) # Wait after ring

        if u == self.sink_node:
            self.update_status_text(f"DFS Path to Sink T={self.sink_node} found!", color=GREEN_B, play_anim=False)
            self.wait(2.0) # Increased wait
        ...
        while self.ptr[u] < len(self.adj[u]):
            ...
            if is_valid_lg_edge:
                ...
                self.wait(1.5) # Increased wait
                if current_anims_try: self.play(*current_anims_try, run_time=0.4) # Slightly longer anim
                self.wait(0.5) # Wait after try animation
                ...
                if current_anims_backtrack_restore: self.play(*current_anims_backtrack_restore, run_time=0.4) # Slightly longer anim
                self.play(Indicate(edge_mo_for_v, color=RED_D, scale_factor=1.0, run_time=0.45)) # Slightly longer anim
                self.wait(0.5) # Wait after indicate
                self.update_status_text(f"DFS Advance: From {u}, exploring next valid LG edge.", play_anim=False) # Clarified "next"
                self.wait(1.0) # Wait before next iteration or ptr increment

            self.ptr[u] += 1

        self.update_status_text(f"DFS Retreat: All LG edges from {u} explored. Backtracking from {u}.", color=ORANGE, play_anim=False) # Clarified
        self.wait(2.0) # Increased wait
        self.play(FadeOut(highlight_ring), run_time=0.15)
        if highlight_ring in self.dfs_traversal_highlights: self.dfs_traversal_highlights.remove(highlight_ring)
        return 0
    ...
    def animate_dfs_path_finding_phase(self):
        ...
        while True:
            ...
            self.wait(1.5) # Wait after announcing attempt
            current_path_anim_info = []
            # The recursive DFS will have its own waits
            ...
            self.wait(2.5) # Increased wait before path highlight
            ...
            if path_highlight_anims: self.play(AnimationGroup(*path_highlight_anims, lag_ratio=0.15, run_time=1.0)) # Increased run_time
            self.wait(2.0) # Increased wait after path highlight
            ...
            if text_update_anims or augmentation_anims:
                 self.play(AnimationGroup(*(text_update_anims + augmentation_anims), lag_ratio=0.1), run_time=1.5) # Increased run_time
            else: self.wait(0.1)
            self.wait(0.5) # Wait after augmentation animations

            self.update_status_text(f"Flow augmented. Current phase flow: {total_flow_this_phase:.1f}. Searching next path...", color=WHITE, play_anim=True) # play_anim was missing, added True
            self.wait(2.5) # Increased wait
    ...
    def construct(self):
        ...
        # No explicit wait after these initial non-animated texts; next is graph build animations
        ...
        self.update_status_text("Source (S) and Sink (T) identified.", play_anim=True); self.wait(2.5) # Increased wait

        self.update_section_title("2. Running Dinitz's Algorithm", play_anim=True) # play_anim=True for visual change
        self.wait(1.0) # Wait after section title
        ...
        while True:
            ...
            self.update_phase_text(f"Phase {self.current_phase_num}: Step 1 - Build Level Graph (LG)", color=BLUE_B, play_anim=True) # play_anim=True
            self.wait(1.0) # Wait after phase text
            ...
            self.play(Write(first_level_text_group)); self.wait(1.0) # Wait after writing level
            ...
            for edge_key, edge_mo in self.edge_mobjects.items():
                ...
                if label_grp and label_grp.submobjects:
                    base_opacity = self.base_label_visual_attrs.get(edge_key, {}).get("opacity", 0.0)
                    if edge_key in self.original_edge_tuples : base_opacity = 1.0
                    ...
                    for part_idx, part in enumerate(label_grp.submobjects):
                        ...
                        if base_opacity > 0 and edge_key in self.original_edge_tuples:
                            ...
                            # Ensure residual capacities on non-original edges that are not LG start transparent
                             if not (self.levels.get(edge_key[1], -1) == self.levels.get(edge_key[0], -1) + 1 and (self.capacities.get(edge_key,0) - self.flow.get(edge_key,0)) > 0) :
            ...
            while q_bfs:
                ...
                for u_bfs in nodes_this_level:
                        ...
                        if edge_mo_bfs and res_cap_bfs > 0 and self.levels[v_n_bfs] == -1:
                            ...
                            if label_grp_bfs and label_grp_bfs.submobjects:
                                if edge_key_bfs not in self.original_edge_tuples:
                                    res_cap_mobj = label_grp_bfs.submobjects[0]
                    ...
                    self.play(FadeOut(ind_u), run_time=0.20) # Slightly longer

                if bfs_anims_this_step: self.play(AnimationGroup(*bfs_anims_this_step, lag_ratio=0.1), run_time=0.8); self.wait(0.5) # Increased run_time and wait
                ...
                if nodes_found_next_level_set:
                    ...
                    self.wait(0.5) # Wait after status update
                    ...
                    self.play(Write(new_level_text_entry)); self.wait(1.5) # Increased wait
            ...
            if self.levels[self.sink_node] == -1:
                ...
                self.wait(4.5) # Increased wait
                break
            else:
                self.update_status_text(f"Sink T={self.sink_node} at L{self.levels[self.sink_node]}. Level Graph layers established.", color=GREEN_A, play_anim=True); self.wait(3.0) # Increased wait
                self.update_status_text(f"Isolating LG: Edges $(u,v)$ where $level(v)=level(u)+1$ & residual capacity $>0$.", play_anim=True)
                self.wait(1.0) # Wait before LG isolation animation
                lg_iso_anims = []
                for (u_lg,v_lg), edge_mo_lg in self.edge_mobjects.items():
                    ...
                    if is_lg_edge:
                        ...
                        if label_grp_lg and label_grp_lg.submobjects:
                            if (u_lg,v_lg) not in self.original_edge_tuples:
                                lg_iso_anims.append(label_grp_lg.submobjects[0].animate.set_opacity(0.0))
                            else:
                                for part in label_grp_lg.submobjects: lg_iso_anims.append(part.animate.set_opacity(DIMMED_OPACITY))

                if lg_iso_anims: self.play(AnimationGroup(*lg_iso_anims, lag_ratio=0.05), run_time=1.0) # Increased run_time
                self.wait(2.0) # Increased wait
                self.update_status_text("Level Graph isolated. Ready for DFS phase.", color=GREEN_A, play_anim=True); self.wait(2.5) # Increased wait
                ...
                self.wait(3.5) # Increased wait
                if self.levels[self.sink_node] != -1 : # Check if sink was reached in THIS phase's BFS
                     self.update_status_text(f"Phase complete. Preparing for next phase (if any).", color=BLUE_A, play_anim=True)
                     self.wait(3.0) # Increased wait

        # This fallback should ideally not be hit if the BFS logic correctly breaks the loop
        if self.levels[self.sink_node] != -1 and self.max_flow_value > 0 and ( hasattr(self, 'flow_this_phase') and self.flow_this_phase == 0 ) :
            # This case could happen if BFS finds a path to sink, but DFS finds no path (e.g. dead ptrs from previous phases blocking flow)
            # which means the previous phase's blocking flow was indeed blocking for *that* level graph.
            # The algorithm should terminate via the "Sink not reached by BFS" path if no more LGs can be built.
             pass # No explicit message here, termination is handled by BFS check

        self.update_section_title("3. Dinitz Algorithm Summary", play_anim=True)
        self.wait(1.0) # Wait after section title
        self.update_status_text(f"Algorithm concluded. Max flow: {self.max_flow_value:.1f}", color=GREEN_A, play_anim=True)
        self.wait(5.0) # Increased wait
        ...
        final_mobjects_to_keep = [
            ...
            self.network_display_group, self.source_ring_mobj, self.sink_ring_mobj
        ]
        ...
        for item in final_mobjects_to_keep:
            if item:
                for sub_item in item.get_family(): # Use get_family to include all submobjects recursively
                    kept_mobjects_set.add(sub_item)

        for mobj in current_scene_mobjects:
            if mobj not in kept_mobjects_set:
                mobjects_to_fade_out.add(mobj) # Add mobj itself

        if hasattr(self, 'dfs_traversal_highlights') and self.dfs_traversal_highlights in self.mobjects:
            if self.dfs_traversal_highlights not in kept_mobjects_set: # Check if not already included
                 mobjects_to_fade_out.add(self.dfs_traversal_highlights)


        if mobjects_to_fade_out.submobjects:
            self.play(FadeOut(mobjects_to_fade_out), run_time=1.0)
        self.wait(10) # Increased final wait significantly