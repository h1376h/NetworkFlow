...
class DinitzAlgorithmVisualizer(Scene):
    ...
    def _format_number(self, value, default_precision=1):
        """
        Formats a number. If it's a whole number, returns its integer string representation.
        Otherwise, formats it as a float string with default_precision.
        """
    ...
    def display_calculation_details(self, path_info=None, bottleneck_value=None, play_anim=True, verbose_bottleneck=False):
        """
        ...
        - verbose_bottleneck: If True, shows edge details in min calculation (default: False)
        """
        ...
        # Generate the bottleneck calculation text
        details_parts = []
        ...
        for (u, v), _ in path_info:
            ...
            if verbose_bottleneck:
                ...
                details_parts.append(f"{formatted_res_cap} on {u_display}→{v_display}")
            else:
                details_parts.append(formatted_res_cap)
        
        formatted_bottleneck_value = self._format_number(bottleneck_value)
        calculation_str = f"Bottleneck = min({', '.join(details_parts)}) = {formatted_bottleneck_value}"
        ...
        while self.ptr[u] < len(self.adj[u]):
            ...
            if is_valid_lg_edge:
                ...
                if edge_key_uv not in self.original_edge_tuples:
                    ...
                    if label_mobj:
                        target_label = Text(self._format_number(res_cap_cand), font=label_mobj.font, font_size=label_mobj.font_size, color=YELLOW_A)
                ...
                self.update_status_text(f"Advance: Try edge ({u_display_name} -> {actual_v_display_name}), Res.Cap: {self._format_number(res_cap_cand)}.", play_anim=False)
                self.wait(1.5)
                ...
                if is_still_lg_edge_after_fail: # Restore to LG appearance
                    ...
                    if edge_key_uv not in self.original_edge_tuples: # Restore residual capacity label
                        label_mobj = self.edge_residual_capacity_mobjects.get(edge_key_uv)
                        if label_mobj:
                            target_label_revert = Text(self._format_number(current_res_cap_after_fail), font=label_mobj.font, font_size=label_mobj.font_size, color=lg_color)
    ...
    def animate_dfs_path_finding_phase(self):
        ...
        self.display_calculation_details(None, None, play_anim=False) # Uses default verbose_bottleneck=False
        ...
        while True: # Loop to find multiple paths in the current LG
            ...
            # Display bottleneck calculation (default verbose_bottleneck=False)
            # To use verbose: self.display_calculation_details(current_path_anim_info, bottleneck_flow, play_anim=True, verbose_bottleneck=True)
            ...
            if bottleneck_edges_for_indication:
                self.update_status_text(f"Path #{path_count_this_phase} found. Bottleneck: {self._format_number(bottleneck_flow)}. Identifying bottleneck edges...", color=YELLOW_A, play_anim=True)
                self.wait(1.0)
                flash_anims = [
            ...
            self.update_status_text(f"Path #{path_count_this_phase} found. Bottleneck: {self._format_number(bottleneck_flow)}. Augmenting flow...", color=GREEN_A, play_anim=True)
            self._update_sink_action_text("augment")
            self.wait(1.0)
            ...
            for (u,v), edge_mo in current_path_anim_info:
                ...
                if (u,v) in self.original_edge_tuples:
                    ...
                    new_flow_str_uv = self._format_number(new_flow_val_uv)
                ...
                if not is_still_lg_edge_uv: # Edge is saturated or no longer LG
                    ...
                    if (u,v) not in self.original_edge_tuples:
                        label_mobj_uv = self.edge_residual_capacity_mobjects.get((u,v))
                        if label_mobj_uv:
                            target_label_uv = Text(self._format_number(res_cap_after_uv), font=label_mobj_uv.font, font_size=label_mobj_uv.font_size, color=lg_color_uv)
                ...
                if (v,u) in self.edge_mobjects:
                    ...
                    if (v,u) not in self.original_edge_tuples:
                        ...
                        if label_mobj_vu: # This is a residual capacity label for a non-original edge
                            # Update its text if it becomes visible or changes, or ensure it's hidden
                            if res_cap_vu > 0: # If it has capacity, make it visible and update text
                                target_label_vu_text = self._format_number(res_cap_vu)
                                # Determine color (e.g., LG color if it becomes part of LG, else default)
                                # For simplicity, let's assume it just becomes visible with a default color or LG color if applicable.
                                # Here, just making sure it's updated.
                                target_label_vu = Text(target_label_vu_text, font=label_mobj_vu.font, font_size=label_mobj_vu.font_size, color=REVERSE_EDGE_COLOR) # Or an LG color
                                target_label_vu.move_to(label_mobj_vu.get_center()).set_opacity(1.0) # Make it visible
                            ...
                            else: # No capacity, ensure it's hidden
                                visual_updates_this_edge.append(label_mobj_vu.animate.set_opacity(0.0))

                    else: # (v,u) is an original edge, update its flow value
                        ...
                        if old_rev_flow_text_mobj:
                            ...
                            new_rev_flow_str_vu = self._format_number(new_rev_flow_val_vu)
            ...
            self.update_status_text(f"Flow augmented. Current phase flow: {self._format_number(total_flow_this_phase)}. Searching for next path...", color=WHITE, play_anim=True)
            self.wait(2.5)
    ...
    def construct(self):
        ...
        for v_id in self.vertices_data:
            ...
            label_str = "s" if v_id == self.source_node else "t" if v_id == self.sink_node else str(v_id) # Initial label is number
        ...
        for u, v, cap in self.edges_with_capacity_list: # Original edges
            ...
            # Initial flow is 0, capacity is cap. Use _format_number for consistency if cap could be float.
            # Assuming cap is integer as per example.
            flow_val_mobj = Text(self._format_number(0), font_size=EDGE_FLOW_PREFIX_FONT_SIZE, color=LABEL_TEXT_COLOR)
            slash_mobj = Text("/", font_size=EDGE_FLOW_PREFIX_FONT_SIZE, color=LABEL_TEXT_COLOR)
            cap_text_mobj = Text(str(cap), font_size=EDGE_CAPACITY_LABEL_FONT_SIZE, color=LABEL_TEXT_COLOR) # Assuming cap is int
        ...
        while True: # Main loop for Dinitz phases
            ...
            for v_id, node_group in self.node_mobjects.items():
                ...
                # Label color was already set to "s" and "t" specific, or base for others.
                # If s/t had special color due to leveling, ensure it's base_node_visual_attrs label color
                if v_id == self.source_node or v_id == self.sink_node:
                     restore_anims.append(lbl.animate.set_color(self.base_node_visual_attrs[v_id]["label_color"]))
                else:
                    ...
                    for v_n_bfs in sorted_neighbors_bfs:
                        ...
                        if edge_mo_bfs and res_cap_bfs > 0 and self.levels[v_n_bfs] == -1:
                            ...
                            if edge_key_bfs not in self.original_edge_tuples:
                                ...
                                if res_cap_mobj:
                                    target_text = Text(self._format_number(res_cap_bfs), font=res_cap_mobj.font, font_size=res_cap_mobj.font_size, color=edge_color_u_for_lg)
                                ...
                                if label_grp_bfs:
                                    # Make sure original label parts (flow/cap) are visible and colored appropriately
                                    bfs_anims_this_step.append(label_grp_bfs.animate.set_opacity(1.0))
                                    for part_idx, part_mobj in enumerate(label_grp_bfs.submobjects):
                                        # Flow value (index 0) and slash (index 1) could be colored like the edge, capacity (index 2) default
                                        if isinstance(part_mobj, Text):
                                            bfs_anims_this_step.append(part_mobj.animate.set_color(edge_color_u_for_lg if part_idx < 2 else LABEL_TEXT_COLOR))
            ...
            if self.levels[self.sink_node] == -1:
                ...
                self.wait(3.0)
                self.update_max_flow_display(play_anim=True)
                self.update_phase_text(f"Algorithm Complete. Max Flow: {self._format_number(self.max_flow_value)}", color=TEAL_A, play_anim=True)
                self.update_status_text(f"Final Max Flow: {self._format_number(self.max_flow_value)}", color=GREEN_A, play_anim=True)
                self._update_sink_action_text("nothing", animate=False)
                self.wait(4.5)
                break
            else:
                    ...
                    if is_lg_edge:
                        ...
                        if label_grp_lg and label_grp_lg.submobjects:
                            if (u_lg,v_lg) not in self.original_edge_tuples:
                                ...
                                if res_cap_mobj:
                                    target_text = Text(self._format_number(res_cap_lg_val), font=res_cap_mobj.font, font_size=res_cap_mobj.font_size, color=lg_color)
                                    if hasattr(self, 'scaled_flow_text_height') and self.scaled_flow_text_height: target_text.height = self.scaled_flow_text_height * 0.9
                                    target_text.move_to(res_cap_mobj.get_center()).set_opacity(1.0)
                                    lg_iso_anims.append(res_cap_mobj.animate.become(target_text))
                            else: # Original edge label
                                lg_iso_anims.append(label_grp_lg.animate.set_opacity(1.0))
                                # Color original label parts appropriately
                                for part_idx, part_mobj in enumerate(label_grp_lg.submobjects):
                                    if isinstance(part_mobj, Text):
                                        lg_iso_anims.append(part_mobj.animate.set_color(lg_color if part_idx < 2 else LABEL_TEXT_COLOR))
                        ...
                        if label_grp_lg and label_grp_lg.submobjects:
                            if (u_lg,v_lg) not in self.original_edge_tuples: # Non-original edge's residual capacity label
                                lg_iso_anims.append(label_grp_lg.animate.set_opacity(0.0))
                            else: # Original edge's flow/capacity label
                                lg_iso_anims.append(label_grp_lg.animate.set_opacity(DIMMED_OPACITY))
                                for part_mobj in label_grp_lg.submobjects: # Revert original label parts to default color
                                     if isinstance(part_mobj, Text):
                                        lg_iso_anims.append(part_mobj.animate.set_color(LABEL_TEXT_COLOR))
                ...
                self._update_sink_action_text("nothing", animate=False)
                self.update_phase_text(f"End of Phase {self.current_phase_num}. Blocking Flow: {self._format_number(flow_this_phase)}. Total Flow: {self._format_number(self.max_flow_value)}", color=TEAL_A, play_anim=True)
                self.wait(3.5)
        ...
        if self.levels.get(self.sink_node, -1) == -1 and self.max_flow_value == 0 :
            self.update_status_text(f"Algorithm Concluded. Sink Unreachable. Max Flow: {self._format_number(self.max_flow_value)}", color=RED_A, play_anim=True)
        elif self.levels.get(self.sink_node, -1) == -1 :
            self.update_status_text(f"Algorithm Concluded. Sink Unreachable in last BFS. Final Max Flow: {self._format_number(self.max_flow_value)}", color=GREEN_A, play_anim=True)
        else: # Should not happen if loop broke due to sink unreachable
            self.update_status_text(f"Algorithm Concluded. Final Max Flow: {self._format_number(self.max_flow_value)}", color=GREEN_A, play_anim=True)
        self.wait(5.0)
        ...
        # Ensure all elements in mobjects_that_should_remain_on_screen are actual Mobjects
        # This is a bit of a workaround for potential non-Mobject items if groups were altered unexpectedly
        valid_kept_mobjects = Group()
        for m_item in mobjects_that_should_remain_on_screen:
            if isinstance(m_item, Mobject):
                valid_kept_mobjects.add(m_item)
                
        final_mobjects_to_fade_out = Group()
        all_descendants_of_kept_mobjects = set()
        for mobj_to_keep in valid_kept_mobjects: # Use the validated group
        ...
        if final_mobjects_to_fade_out.submobjects: # Check if there's anything to fade
