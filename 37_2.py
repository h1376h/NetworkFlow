...
def create_flow_graph(step_num, title, flows_data, residual_data, initial_caps_ref, path_edges=None):
    """
    Creates a Digraph for a specific step of the Ford-Fulkerson algorithm.

    Args:
        step_num (int): The current step number.
        title (str): A descriptive title for the step.
    ...
    dot.attr(label=f'{title}\nTotal Flow: {flows_data["total_flow"]:.5f}', labelloc='t', fontsize='20')
    ...
    # This loop ensures we only draw each reverse edge once and correctly
...
def run_ford_fulkerson_irrational_loop_steps():
    ...
    # Flow on original edges, starting at 0
    current_flows = {edge: 0.0 for edge in initial_caps}
    
    # Residual capacities initially are just the original capacities
    # Plus, initialize reverse edges with 0 residual capacity for all possible reverse pairs
    ...
    create_flow_graph(step_count, "Step 0: Initial State (f(e)=0)", current_flows.copy(), residual_capacities.copy(), initial_caps, [])
    step_count += 1

    # Loop Iteration 1 (Augment P1: s -> x1 -> x2 -> t)
    path_edges = [('s', 'x1'), ('x1', 'x2'), ('x2', 't')]
    # This path exists because s->x1 (1), x1->x2 (1), x2->t (phi) are all available.
    bottleneck = min(residual_capacities[e] for e in path_edges) # min(1, 1, phi) = phi

    # Augment flow along the path
    for u, v in path_edges:
        current_flows[(u, v)] += bottleneck # Add flow to original edge
        residual_capacities[(u, v)] -= bottleneck # Reduce forward residual capacity
        residual_capacities[(v, u)] += bottleneck # Increase reverse residual capacity
    total_network_flow += bottleneck

    current_flows['total_flow'] = total_network_flow
    create_flow_graph(step_count, f"Step {step_count}: Augment s->x1->x2->t by {bottleneck:.5f}",
    ...
    # Bottleneck on this path: min(c_f(s,x2), c_f(x2,x1), c_f(x1,t))
    ...
    current_flows['total_flow'] = total_network_flow
    create_flow_graph(step_count, f"Step {step_count}: Augment s->x2->x1->t by {bottleneck:.5f}",
                      current_flows.copy(), residual_capacities.copy(), initial_caps, path_edges)
    step_count += 1
    ...
    current_flows['total_flow'] = total_network_flow
    create_flow_graph(step_count, f"Step {step_count}: Augment s->x1->t by {bottleneck:.5f}",
                      current_flows.copy(), residual_capacities.copy(), initial_caps, path_edges)
    step_count += 1
    ...
    # Step 4: Now, a path involving ('x2', 't') becomes available again, but with a smaller capacity.
    # This involves flow being pushed back on ('x1', 'x2') to open up capacity.
    # Conceptually, a cycle of `phi`, `phi`, `phi^2` is completed, then `phi^3`, `phi^3`, `phi^4`, etc.
    
    # Manually reset relevant residual capacities to continue the pattern for visualization
    # This is not derived from strict prior steps, but shows the *pattern* of the loop.
    
    # Simulate the result of a path that leads to a bottleneck of phi^3
    bottleneck_for_cycle = PHI**3 
    
    # Path similar to step 1 (s -> x1 -> x2 -> t)
    path_edges = [('s', 'x1'), ('x1', 'x2'), ('x2', 't')]
    ...
    current_flows['total_flow'] = total_network_flow
    create_flow_graph(step_count, f"Step {step_count}: Augment s->x1->x2->t by {bottleneck_for_cycle:.5f} (Loop Cycle Part 4)",
                      current_flows.copy(), residual_capacities.copy(), initial_caps, path_edges)
    step_count += 1
    ...
    total_network_flow += bottleneck_for_cycle
    current_flows['total_flow'] = total_network_flow
    create_flow_graph(step_count, f"Step {step_count}: Augment s->x2->x1->t by {bottleneck_for_cycle:.5f} (Loop Cycle Part 5)",
                      current_flows.copy(), residual_capacities.copy(), initial_caps, path_edges)
    step_count += 1
    ...
    current_flows['total_flow'] = total_network_flow
    create_flow_graph(step_count, f"Step {step_count}: Augment s->x1->t by {bottleneck_for_cycle:.5f} (Loop Cycle Part 6)",
                      current_flows.copy(), residual_capacities.copy(), initial_caps, path_edges)
    step_count += 1
    ...
    for i in range(7, 10):
        ...
        current_flows['total_flow'] = total_network_flow
        create_flow_graph(step_count, f"Step {step_count}: Augment by {bottleneck_for_cycle:.5f} (Loop Cycle Part {i})",
                          current_flows.copy(), residual_capacities.copy(), initial_caps, path_edges)
        step_count += 1
