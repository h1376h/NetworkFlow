...
class DinitzAlgorithmVisualizer(Scene):
    ...
    def _update_text_generic(self, text_attr_name, new_text_str, font_size, weight, color, play_anim=True):
        ...
        else: # Quick update without animation, just swap
        ...
        while True:
            ...
            for (u,v), edge_mo, orig_color, orig_width, orig_opacity in current_path_anim_info:
                ...
                if (v,u) in self.edge_mobjects:
                    ...
                    rev_label_grp = self.edge_label_groups.get((v,u))
                    ...
                    if is_rev_edge_in_lg:
                        ...
                        augmentation_anims.append(rev_edge_mo.animate.set_stroke(opacity=0.7, width=EDGE_STROKE_WIDTH , color=GREY_A))
                        if rev_label_grp: 
                            for part in rev_label_grp.submobjects: augmentation_anims.append(part.animate.set_opacity(0.7))
                    else:
                        base_rev_edge_attrs = self.base_edge_visual_attrs.get((v,u))
                        base_rev_label_attrs = self.base_label_visual_attrs.get((v,u))
                        if base_rev_edge_attrs:
                            augmentation_anims.append(rev_edge_mo.animate.set_stroke(opacity=base_rev_edge_attrs["opacity"], width=base_rev_edge_attrs["stroke_width"], color=base_rev_edge_attrs["color"]))
                        if rev_label_grp and base_rev_label_attrs: 
                             for part in rev_label_grp.submobjects: augmentation_anims.append(part.animate.set_opacity(base_rev_label_attrs["opacity"]))
    ...
    def construct(self):
        ...
        for u_node in self.vertices_data:
            for v_node in self.adj[u_node]:
                ...
                if current_edge_tuple not in original_edge_tuples and current_edge_tuple not in self.edge_mobjects:
                    ...
                    self.edge_flow_val_text_mobjects[current_edge_tuple] = flow_val_mobj
                    self.edge_slash_text_mobjects[current_edge_tuple] = slash_mobj
                    self.edge_capacity_text_mobjects[current_edge_tuple] = cap_text_mobj
                    self.base_label_visual_attrs[current_edge_tuple] = {"opacity": REVERSE_LABEL_OPACITY}
                    ...
                    self.edge_label_groups[current_edge_tuple] = rev_label_group
        ...
        if self.edge_flow_val_text_mobjects:
            ...
            if not sample_key_options and self.edge_flow_val_text_mobjects:
            ...
            if sample_key_options:
                ...
                self.scaled_flow_text_height = dummy_text.scale(self.desired_large_scale).height
        else:
            dummy_text = Text("0", font_size=EDGE_FLOW_PREFIX_FONT_SIZE)
            self.scaled_flow_text_height = dummy_text.scale(self.desired_large_scale).height
        ...
        for edge_key, edge_mo in self.edge_mobjects.items():
            ...
            if edge_key in self.edge_label_groups and edge_key not in self.base_label_visual_attrs:
                 initial_label_opacity = self.edge_label_groups[edge_key].get_opacity()
                 if initial_label_opacity is None: initial_label_opacity = 1.0
                 self.base_label_visual_attrs[edge_key] = {"opacity": initial_label_opacity}
        ...
        while True:
            ...
            for edge_key, edge_attrs in self.base_edge_visual_attrs.items():
                ...
                if edge_key in self.edge_label_groups:
                    ...
                    if label_base_attrs:
                        base_opacity = label_base_attrs.get("opacity", 1.0)
                        for label_part_mo in self.edge_label_groups[edge_key].submobjects:
                            restore_anims.append(label_part_mo.animate.set_opacity(base_opacity))
            if restore_anims:
            ...
            while q_bfs:
                ...
                for u_bfs in nodes_to_process_this_level:
                    ...
                    ind_u_rect_color = YELLOW_C
                    ind_u = SurroundingRectangle(node_to_indicate, color=ind_u_rect_color, buff=0.03, stroke_width=2.0, corner_radius=0.05)
                            ...
                            if edge_mo_bfs:
                                 ...
                                 if (u_bfs, v_n_bfs) in self.edge_label_groups:
                                     ...
                                     for part in self.edge_label_groups[(u_bfs, v_n_bfs)].submobjects:
                                         edge_highlight_anims_bfs_step.append(part.animate.set_opacity(target_opacity))
            ...
            if self.levels[self.sink_node] == -1:
                ...
                for (u_lg,v_lg), edge_mo_lg in self.edge_mobjects.items():
                    ...
                    if is_lg_edge:
                        ...
                        base_attrs_edge = self.base_edge_visual_attrs.get((u_lg,v_lg))
                        base_attrs_label = self.base_label_visual_attrs.get((u_lg,v_lg))

                        target_edge_opacity = DIMMED_OPACITY
                        target_edge_color = DIMMED_COLOR
                        target_label_opacity = DIMMED_OPACITY 

                        if is_pure_reverse_edge and base_attrs_edge:
                            target_edge_opacity = base_attrs_edge.get("opacity", REVERSE_EDGE_OPACITY)
                            target_edge_color = base_attrs_edge.get("color", REVERSE_EDGE_COLOR)
                            if base_attrs_label:
                                target_label_opacity = base_attrs_label.get("opacity", REVERSE_LABEL_OPACITY)
                        
                        current_edge_opacity = edge_mo_lg.get_stroke_opacity()
                        if current_edge_opacity is None: current_edge_opacity = 1.0 # Default if None
                        current_edge_color_rgb = color_to_rgb(edge_mo_lg.get_color())
                        target_edge_color_rgb = color_to_rgb(target_edge_color)

                        if not np.isclose(current_edge_opacity, target_edge_opacity) or \
                           not np.allclose(current_edge_color_rgb, target_edge_color_rgb):
                            non_lg_edge_anims_iso.append(edge_mo_lg.animate.set_stroke(opacity=target_edge_opacity, color=target_edge_color))
                        
                        if edge_label_grp and edge_label_grp.submobjects and base_attrs_label:
                            current_label_part_opacity = edge_label_grp.submobjects[0].get_opacity()
                            
                            if current_label_part_opacity is None: 
                                current_label_part_opacity = 1.0 

                            if not np.isclose(current_label_part_opacity, target_label_opacity):
                                for part in edge_label_grp.submobjects: 
                                    non_lg_edge_anims_iso.append(part.animate.set_opacity(target_label_opacity))
                ...
                if self.levels[self.sink_node] != -1 :
                    self.update_status_text(f"Blocking flow of {flow_this_phase:.1f} found. Current Max Flow: {self.max_flow_value:.1f}. Preparing for next phase.", color=BLUE_A, play_anim=True)
                    self.wait(1.5)
        ...
        final_mobjects_to_keep_list_end = [
            ...
            self.algo_status_mobj, self.level_display_vgroup, self.network_display_group,
            self.source_ring_mobj, self.sink_ring_mobj
        ]
        
        mobjects_to_fade_out_finally = Group() # Changed from VGroup to Group
        current_scene_mobjects = self.mobjects.copy() 

        kept_mobjects_set = set()
        for item in final_mobjects_to_keep_list_end:
            if item: 
                kept_mobjects_set.add(item)
                if isinstance(item, Mobject): # Check if item itself is an Mobject before calling get_family
        ...
        for mobj in current_scene_mobjects:
            if mobj not in kept_mobjects_set:
                mobjects_to_fade_out_finally.add(mobj) # This should now work with Group
