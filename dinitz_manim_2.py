...
class DinitzAlgorithmVisualizer(Scene):
    ...
    def _update_text_generic(self, text_attr_name, new_text_content, font_size, weight, color, play_anim=True, is_latex=False):
        ...
        if play_anim:
            ...
            if is_empty_new_content:
                # If an empty text is not in the info group and is on scene,
                # it might be removed by this logic.
                # However, self.sink_action_text_mobj is handled by its own update function.
                if not is_in_group and new_mobj in self.mobjects:
                     # self.remove(new_mobj) # Decide if empty generic texts should be auto-removed.
                                            # For now, let them exist but be empty.
                     pass
            else:
    ...
    def _update_sink_action_text(self, new_text_content, new_color=YELLOW, animate=True):
        ...
        if animate:
            ...
            current_mobj.become(target_text_template) 
            if current_mobj not in self.mobjects: # Should always be in self.mobjects after initial add
    ...
    def animate_dfs_path_finding_phase(self):
        ...
        self._update_sink_action_text("", animate=False) # Text will be above source (empty)
    ...
    def construct(self):
        ...
        while True:
            ...
            if self.levels[self.sink_node] == -1:
                ...
                self.wait(3.5)
                if self.levels.get(self.sink_node, -1) != -1 : # Check if sink was reachable in this phase's BFS
