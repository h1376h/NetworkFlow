...
BOTTLENECK_CALCULATION_NUMBER_COLOR = BLUE_B # Added for bottleneck numbers
...
class DinitzAlgorithmVisualizer(Scene):
    ...
    def setup_titles_and_placeholders(self):
        ...
        self.calculation_details_mobj = Text("", font_size=STATUS_TEXT_FONT_SIZE).set_z_index(10) # Will become MarkupText if needed
    ...
    def _animate_text_update(self, old_mobj, new_mobj, new_text_content_str):
        ...
        elif isinstance(old_mobj, MarkupText) and old_mobj.text != "": old_text_had_actual_content = True # MODIFIED: Check for MarkupText
    ...
    def _update_text_generic(self, text_attr_name, new_text_content, font_size, weight, color, play_anim=True, is_latex=False, is_markup=False): # MODIFIED: Added is_markup
        old_mobj = getattr(self, text_attr_name)

        if is_markup: # MODIFIED: Handle MarkupText
            ...
            if ref_text_for_height.height > 0.001 and new_mobj.height > 0.001 and new_mobj.tex_string: # check tex_string
        ...
        elif old_mobj in self.mobjects: # Not in group but on scene
             new_mobj.move_to(old_mobj.get_center())
    ...
    def update_status_text(self, text_str, color=WHITE, play_anim=True, is_latex=False):
        # MODIFIED: Pass is_markup=False explicitly for this one as it's not changing to MarkupText here
    ...
    def update_max_flow_display(self, play_anim=True):
        # MODIFIED: Use _format_number
        new_text_str = self._format_number(self.max_flow_value) # Default precision 1 if float
